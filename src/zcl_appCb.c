...
//    printf("app_zclWriteReqCmd()\r\n");
    ...
    if (clusterId == ZCL_CLUSTER_SE_METERING) {
        for (uint8_t i = 0; i < numAttr; i++) {
            if (attr[i].attrID == ZCL_ATTRID_CUSTOM_SERIAL_NUMBER && attr[i].dataType == ZCL_DATA_TYPE_OCTET_STR) {
                if (memcmp(dev_config.serial_number, attr[i].attrData, attr[i].attrData[0] + 1) != 0) {
                    memcpy(dev_config.serial_number, attr[i].attrData, attr[i].attrData[0] + 1);
                    write_config();
//                    zcl_setAttrVal(APP_ENDPOINT1, ZCL_CLUSTER_SE_METERING, ZCL_ATTRID_CUSTOM_SERIAL_NUMBER, (uint8_t*)attr[i].attrData);
                    zcl_setAttrVal(APP_ENDPOINT1, ZCL_CLUSTER_SE_METERING, ZCL_ATTRID_METER_SERIAL_NUMBER, (uint8_t*)attr[i].attrData);
#if UART_PRINTF_MODE
                    printf("New serial number: %s\r\n", print_str_zcl(dev_config.serial_number));
#endif
                }
            } else if (attr[i].attrID == ZCL_ATTRID_CUSTOM_DEVICE_NAME && attr[i].dataType == ZCL_DATA_TYPE_OCTET_STR) {
                if (memcmp(dev_config.device_name, attr[i].attrData, attr[i].attrData[0] + 1) != 0) {
                    memcpy(dev_config.device_name, attr[i].attrData, attr[i].attrData[0] + 1);
                    write_config();
#if UART_PRINTF_MODE
                    printf("New device name: %s\r\n", print_str_zcl(dev_config.device_name));
#endif
                }
            } else if (attr[i].attrID == ZCL_ATTRID_CUSTOM_ENERGY_PRESET && attr[i].attrData) {
                uint32_t over;
                uint64_t energy = BUILD_U48(attr[i].attrData[0],
                                               attr[i].attrData[1],
                                               attr[i].attrData[2],
                                               attr[i].attrData[3],
                                               attr[i].attrData[4],
                ...
                if (dev_config.number_digits == 5) {
                    over = OVERFLOW_DIGITS_5;
                } else if (dev_config.number_digits == 6) {
                    over = OVERFLOW_DIGITS_6;
                } else {
                    over = OVERFLOW_DIGITS_5;
                }

                if (energy > over) energy = over;

                if (dev_config.summ_delivered != energy) {
                    dev_config.summ_delivered = energy;
                    write_config();
#if UART_PRINTF_MODE
                    printf("New energy preset: %d\r\n", energy);
#endif
                    zcl_setAttrVal(APP_ENDPOINT1, ZCL_CLUSTER_SE_METERING, ZCL_ATTRID_CUSTOM_ENERGY_PRESET, (uint8_t*)&energy);
                    zcl_setAttrVal(APP_ENDPOINT1, ZCL_CLUSTER_SE_METERING, ZCL_ATTRID_CURRENT_SUMMATION_DELIVERD, (uint8_t*)&energy);
                }

            } else if (attr[i].attrID == ZCL_ATTRID_CUSTOM_NUMBER_DIGITS && attr[i].attrData) {
                uint8_t digits = attr[i].attrData[0];
                if (dev_config.number_digits != digits) {
                    dev_config.number_digits = digits;
                    write_config();
#if UART_PRINTF_MODE
                    printf("New energy number of digits: %d\r\n", dev_config.number_digits);
#endif
//                    zcl_setAttrVal(APP_ENDPOINT1, ZCL_CLUSTER_SE_METERING, ZCL_ATTRID_CUSTOM_NUMBER_DIGITS, (uint8_t*)&digits);
                }
            } else if (attr[i].attrID == ZCL_ATTRID_CUSTOM_DIVISOR_PRESET && attr[i].attrData) {
                uint16_t divisor = BUILD_U16(attr[i].attrData[0], attr[i].attrData[1]) / seAttrs->divisor;

                if (dev_config.energy_divisor != divisor) {
                    dev_config.energy_divisor = divisor;
                    reset_counter();
#if UART_PRINTF_MODE
                    printf("New energy divisor: %d\r\n", dev_config.energy_divisor * seAttrs->divisor);
#endif
                }
            }
        }
